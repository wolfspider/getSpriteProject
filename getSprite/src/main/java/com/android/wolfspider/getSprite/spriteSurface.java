 ...
 */
public class spriteSurface extends GLSurfaceView implements SurfaceHolder.Callback{


    public spriteSurface(Context context, AttributeSet attr) {
        super(context, attr);
        init();
    }
    ...
    class GLThread extends Thread {
        ...
        private void guardedRun() throws InterruptedException {
             ...
             */
            while (!mDone) {
                spriteProfiler.sSingleton.start(spriteProfiler.PROFILE_FRAME);
                /*
                ...
                synchronized (this) {
                    if (mEvent != null) {
                        spriteProfiler.sSingleton.start(spriteProfiler.PROFILE_SIM);
                        mEvent.run();
                        spriteProfiler.sSingleton.stop(spriteProfiler.PROFILE_SIM);
                    }
                    if (mPaused) {
                        mEglHelper.finish();
                        needStart = true;
                ...
                if ((w > 0) && (h > 0)) {
                    spriteProfiler.sSingleton.start(spriteProfiler.PROFILE_DRAW);
                    /* draw a frame here */
                    mRenderer.drawFrame(gl);
                    spriteProfiler.sSingleton.stop(spriteProfiler.PROFILE_DRAW);

                    /*
                     * Once we're done with GL, we need to call swapBuffers()
                     * to instruct the system to display the rendered frame
                     */
                    spriteProfiler.sSingleton.start(spriteProfiler.PROFILE_PAGE_FLIP);
                    mEglHelper.swap();
                    spriteProfiler.sSingleton.stop(spriteProfiler.PROFILE_PAGE_FLIP);
                }
                spriteProfiler.sSingleton.stop(spriteProfiler.PROFILE_FRAME);
                spriteProfiler.sSingleton.endFrame();
            }
